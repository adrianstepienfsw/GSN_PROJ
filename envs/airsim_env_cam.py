...
class AirSimEnvCAM(BaseEnv):
    ...
    def reset(self):
        self.setup_car()
        self.do_action(1)
        if (self.distance != 0) and (self.state["collision"]):
    ...
    def compute_reward(self):
        ...
        for ii in range(len(pts)):
            for i in range(0, len(pts[ii]) - 1):
                if (np.linalg.norm(car_pt-pts[ii][i]) <  (5 + np.linalg.norm(pts[ii][i + 1]-pts[ii][i]))) and \
                        (np.linalg.norm(car_pt-pts[ii][i+1]) < (5 + np.linalg.norm(pts[ii][i + 1]-pts[ii][i]))):
                    dist = min(
                        dist,
    ...
    def calculate_distance(self):
        ...
        distance = (self.car_state.speed) * time
        return distance
