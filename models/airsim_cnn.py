...
class AirsimCNN(BasePolicy):
    def __init__(self, sess, input_shape, num_actions, reuse=False, is_training=True, name='train'):
        ...
        with tf.variable_scope("policy", reuse=reuse):
            conv1 = conv2d('conv1', tf.cast(self.X_input, tf.float32), num_filters=32, kernel_size=(5, 5),
            ...
            conv3 = conv2d('conv3', conv2, num_filters=128, kernel_size=(3, 3), padding='VALID', stride=(1, 1),
                           initializer=orthogonal_initializer(np.sqrt(2)), activation=tf.nn.relu,
                           is_training=is_training)
